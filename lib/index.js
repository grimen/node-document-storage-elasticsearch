...
ElasticSearch.defaults = ElasticSearch.defaults || {};
ElasticSearch.defaults.url = Storage.env('ELASTICSEARCH_URL')|| 'http://localhost:9200/{db}-{env}'.assign({
  db: 'default',
  env: process.env.NODE_ENV || 'development'
});
ElasticSearch.defaults.options = {
  client: {
    curlDebug: false
  }
};
...
ElasticSearch.prototype.set = function() {
  ...
  self._set(arguments, function(key_values, options, done, next) {
    ...
    var commands = Object.keys(key_values).map(function(k) {
      var resource = self.resource(k).trimmed;

      return {index: options.merge({index: resource.db, type: resource.type, id: resource.id, data: key_values[k]})};
    });
...
ElasticSearch.prototype.get = function() {
  ...
  self._get(arguments, function(keys, options, done, next) {
    keys.each(function(key) {
      ...
      self.client.get(resource.db, resource.id, options, function(error, result, response) {
...
ElasticSearch.prototype.del = function() {
  ...
  self._del(arguments, function(keys, options, done, next) {
    keys.each(function(key) {
      ...
      self.client.delete(resource.db, (resource.type || '_all'), resource.id, options, function(error, response) {
